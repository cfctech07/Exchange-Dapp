...
class App extends Component {
  ...
  async loadBlockchainData(dispatch) {
    ...
    if (!token) {
      window.alert(
        "Token smart contract not detected on the current network. Please select GOERLI TEST NETWORK with Metamask."
      );
      return;
    }
    ...
    if (!exchange) {
      window.alert(
        "Exchange smart contract not detected on the current network. Please select GOERLI TEST NETWORK with Metamask."
      );
      return;
    }
  }
